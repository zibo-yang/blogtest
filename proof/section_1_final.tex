...
If $u=0$, then $\left\|\begin{pmatrix}C&0\\0&0\end{pmatrix}x\right\|_r=0\le\|C\|_{\ell^r_k}\|x\|_r$, so the bound holds. If $u\ne0$,
\[
...
\le \|C\|_{\ell^r_k}.
\]
In both cases the ratio is $\le\|C\|_{\ell^r_k}$; taking the supremum over nonzero $x$ gives $\le\|C\|_{\ell^r_k}$.
